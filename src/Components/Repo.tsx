...
export type RepoProps = { repo: RepoData };

export type RepoData = any

export function Repo(props: RepoProps) {
    const icon = () => {
        if (props.repo.fork == true)
            return <ForkIcon />;
        else if (props.repo.stargazers_count > 1)
            return <StarIcon />;
        else
            return <></>;
    };
    ...
    return <ContentItem
        ...
        icon={icon()}
        desc={props.repo["description"]}
        tags={[props.repo["language"]]} />
}
